...
app.post("/schedule", async (req, res) => {
    if (!req.body || !req.body.name || !req.body.schedule) return res.status(400).send("Missing name or schedule");
    const name = req.body.name.toString();
    const schedule = req.body.schedule;
    Worker.findOne({name: name,}).then((worker) => {
...
app.get("/download", async (req, res) => {
    ...
    if (!worker) return res.status(400).send("Worker not found");
