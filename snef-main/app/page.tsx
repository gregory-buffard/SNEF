        ...
        <main className="w-screen h-screen flex justify-center items-center flex-col space-y-[3vh]">
            ...
            <div className={"flex items-center justify-center space-x-3"}>
                <input
                    ...
                    className="w-[13vw] iP:w-[30vh] bg-white border-2 border-neutral-300 drop-shadow-2xl placeholder-neutral-300 rounded-full text-xl outline-none px-[0.75vw] iP:px-[1.5vh] py-[0.5vh] font-normal text-neutral-800 hover:bg-neutral-300 hover:placeholder-neutral-400 focus:bg-neutral-300 focus:placeholder-neutral-400 transition-all duration-200 ease-in-out text-[2vh] iP:text-[1.5vh]"
                    value={name}
                    onChange={(e) => setName(e.target.value)}
                />
                <a target={"_blank"}
                    href={`https://api.snef.cloud/download/?name=${name}`}
                ><AiOutlineSearch className={"text-[1.5vw] iP:text-[3vh] text-neutral-800 drop-shadow-xl"} /></a>
                <button onClick={handleClick}><AiOutlineForm className={"text-[1.5vw] iP:text-[3vh] text-neutral-800 drop-shadow-xl"} /></button>
            </div>
        </main>
    );
};
