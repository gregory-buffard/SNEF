...
const Page = () => {
        ...
        const [workspaceVisibility, setWorkspaceVisibility] = useState(
            allWorkspaces.reduce((visibilities: {[key:string]:boolean},workspace: any) => {
                ...
                {loading ?
                    <div className={"w-screen h-screen flex justify-center items-center"}><p>Chargement...</p></div> : (
                        <>
                            ...
                            <div className={`absolute top-0 bg-white-low text-green-600 border-neutral-100 border-[0.25vh] flex justify-center items-center space-x-[0.5vw] text-[2vh] px-[1vw] py-[0.5vh] rounded-[0.75vw] iP:rounded-[1vh] transition duration-800 ease-in-out iP:text-[1.25vh] z-10 backdrop-blur-md drop-shadow-2xl ${confirmationBox ? "translate-y-[2vw]" : "-translate-y-[10vw] iP:-translate-y-[10vh]"}`}><TiTick className={`text-[1.5vw] iP:text-[1.75vh] drop-shadow-greenish opacity-0 ${confirmationBox ? 'animateTick' : ''}`} /><p className={"drop-shadow-greenish"}>Les donneés ont été soumisent.</p></div>
                            ...
                            <button type={'button'} onClick={() => setSelectUserDialog(!selectUserDialog)} className={"absolute left-[6vw] iP:left-[18vh] top-[-1vh] bg-neutral-100 px-[0.25vw] py-[0.25vw] rounded-[0.25vw] hover:bg-neutral-300 shadow-inner iP:text-[3vh] iP:px-[1.5vw] iP:py-[1.5vw] iP:rounded-[2vw] group flex justify-center items-center"}>
                                <HiUserGroup />
                                <p className={'absolute text-tag font-medium transition-all duration-500 ease-in-out group-hover:translate-y-[2.5vh] opacity-0 group-hover:opacity-100 bg-neutral-900 bg-opacity-50 text-neutral-100 px-[0.5vw] py-[0.5vh] rounded-[0.5vw] -z-10 iP:hidden backdrop-blur-sm'}>Employés</p>
                            </button>
                            ...
                            {interimWorkers.length > 0 && (
                                <div className={`absolute bg-neutral-100 iP:bg-snef iP:backdrop-blur-xl px-[1vw] py-[1vh] rounded-[0.5vw] drop-shadow-lg iP:drop-shadow-none top-[3vh] iP:top-[-3vh] transition duration-200 ease-in-out left-0 flex flex-col justify-start items-start iP:w-[90vw] iP:h-screen iP:z-10 iP:rounded-r-[2vh] iP:justify-center iP:items-baseline iP:space-y-[2vh] iP:pl-[25%] iP:text-[2vh] iP:text-neutral-100 iP:border-y-[0.25vh] iP:border-r-[0.25vh] iP:border-teal-700 iP:border-opacity-25 space-y-[1vh] ${selectUserDialog ? 'translate-x-[2vw]  iP:translate-x-0' : 'translate-x-[-15vw] iP:translate-x-[-100vw]'}`}>
                                    <button type={"button"} onClick={() => setSelectUserDialog(!selectUserDialog)} className={"hidden iP:block absolute right-[3vw] top-[3vw] text-[3vh] px-[1vw] py-[1vw] text-center hover:text-neutral-300 hover:bg-neutral-900 hover:bg-opacity-50 rounded-full transition-all duration-200 ease-in-out"}><CgClose /></button>
                                    <WorkersList workers={interimWorkers} setSelectedWorker={setSelectedInterimWorker} />
                                </div>
                            )}
                            {selectedInterimWorker && (
                                <>
                                ...
                                <div className={"w-full flex px-[2vw] justify-between items-center text-[0.6vw] iP:text-[1.5vh]"}>
                                    <div className={'flex flex-col justify-start items-start space-y-[1vh]'}>
                                        <div className={"flex justify-center items-center space-x-[1vw] iP:space-x-[2vh] iP:w-[60vw]"}>
                                            <input type={"checkbox"} checked={signature} className={"cursor-pointer"} onClick={() => {setSignature(!signature)}} />
                                            <p>
