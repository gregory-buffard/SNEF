...
import {Data} from "@/app/form/interim/page";
...
weekAgoDate = `${weekAgoDate.getDate()} / ${weekAgoDate.getMonth() + 1} / ${weekAgoDate.getFullYear()}` as string;
...
const Page = () => {
    ...
    useEffect(() => {
        ...
        switch (name.toLowerCase()) {
            case 'undefined':
                ...
                break;
            case 'interim':
                axios.get("https://api.snef.cloud/workers/?interim=true")
                    .then((res) => {
    ...
    useEffect(() => {
        ...
        data.schedule.forEach(workspace => {
            updatedVisibility[workspace.name] = visibleWorkspace[workspace.name] ?? true;
        });
    ...
    const handleNewWorkspace = async (e: React.FormEvent) => {
        ...
        try {
            const res = await axios.post('https://api.snef.cloud/addWorkspace', newWorkspace);
            setData({
                ...data,
                                        ...
                                        if (selGroupWorker !== null) {
                                            try {
                                                ...
                                                if (!isInterim) {
                                                    ...
                                                    setGroupWorker(updatedWorkers[0]);
                                                }
                                            } catch (err) {
                                                console.log(err);
                                            }
